...
qgpd <- function(p, u = 0, sigma = 1, xi = 0, lower.tail = TRUE, log.p = FALSE) {
  ...
  p <- as.numeric(p)
  if (log.p) p <- exp(p)          # accept log-probabilities
  if (!lower.tail) p <- 1 - p     # convert upper-tail to lower-tail
...
The regression line indicates a positive slope, showing an overall upward trend in mean flow. However, interannual fluctuations are evident, with temporary drops around 2018 and 2023. Thus, rather than a smooth gradual increase, the series shows variable but net rising behaviour, suggesting weak non-stationarity in the mean level of flow.
...
@fig-q4-fig3 presents the QQ-plot comparing empirical exceedances (flows above 75 cumecs) with theoretical quantiles from the GPD($\sigma=29.7$,$\xi=0.62$, $u=0$). A perfect GPD fit would show all points lying closely along the 45° dashed line. Here, the lower quantiles align well, but the upper tail deviates upward - empirical values exceed theoretical ones. This upward curvature suggests that the specified shape parameter $\xi$ may underestimate tail heaviness. Thus, the GPD fit is imperfect for the most extreme flows.
...
ggplot(qq_df, aes(theoretical, empirical)) +
  ...
  labs(title = "QQ-plot for Exceedances (>75) vs GPD(σ=29.7, ξ=0.62, u=0)",
...
@fig-q4-fig4 displays the mean excess plot, where mean exceedance is plotted against thresholds. For a correct GPD tail model, the relationship should be approximately linear beyond the threshold. The curve shows modest upward bending beyond u $≈$ 75, implying that the assumed threshold may be slightly misaligned or the tail distribution deviates from strict GPD behaviour.

```{r fig-q4-fig4}
#| echo: false
#| fig-cap: "Mean residual life (MRL) plot. Near-linear behaviour above u≈75 supports a GPD tail model."
#Q4-fig4
...
Overall, annual boxplots and mean trends show generally stable flow distributions with mild year-to-year variation and a slight upward trend, suggesting weak non-stationarity rather than a clear long-term change.
For exceedances above 75 cumecs, the QQ-plot and mean excess plot both indicate heavier tails and upward curvature, indicating that the assumed GPD($\sigma=29.7$, $\xi=0.62$, $u=0$) underestimates extreme flows.
Therefore, while the first assumption of stationarity is partially valid, the second assumption regarding the GPD fit is not supported.
